...
class TimelineWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        for i in range(200):
            ...
            painter.setPen(QPen(QColor(80, 80, 80) if i % 4 == 0 else QColor(50, 50, 50), 1))
            painter.drawLine(int(x), 0, int(x), self.height())

        # 2. ノート
        for note in self.notes_list:
            r = self.get_note_rect(note)
            color = QColor(60, 160, 255) if not note.is_selected else QColor(255, 180, 0)
